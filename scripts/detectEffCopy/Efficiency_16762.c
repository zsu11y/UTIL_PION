...
void Efficiency_16762()
{

// Run 16762 - SHMS only, LH2, 15 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16762_-1.root"); //open a new file for each beam current 
...
// hg cerenkov hists
auto h_hgcer_should = new TH1D ("P.hgcer.npeSum", "SHMS LH2 15uA Efficiency - Run 16762", 300 , 0.1, 40);
...
// ng cerenkov hists
auto h_ngcer_should = new TH1D ("P.ngcer.npeSum", "SHMS LH2 15uA Efficiency - Run 16762", 300 , 0.1, 40);
...
// aerogel hists
auto h_aero_should = new TH1D ("P.aero.npeSum","SHMS LH2 15uA Efficiency - Run 16762", 300, 0.1, 50);
...
// calorimeter hists
auto h_cal_should = new TH1D ("P.cal.etottracknorm", "SHMS LH2 15uA Efficiency - Run 16762", 300, 0.1, 2);
...
{ 
    if(line.find("run 16762 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16762 - hgcer: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
    if(line.find("run 16762 - ngcer:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16762 - aero: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
}  
...
{
    eff_file_out << "run 16762 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16762 - hgcer: "<< fixed << setprecision(5) << effhgcer << " +/- "  << setprecision(5)<< dEffhgcer << endl;
    eff_file_out << "run 16762 - ngcer: "<< fixed << setprecision(5) << effngcer << " +/- "  << setprecision(5)<< dEffngcer << endl;
    eff_file_out << "run 16762 - aero: "<< fixed << setprecision(5) << effaero << " +/- "  << setprecision(5)<< dEffaero << endl << endl;
    foundhgcer = true;
    foundngcer = true;
    foundcal = true;
    foundaero = true;
...
calLegend->AddEntry(h_cal_should, "Integral = 1012939.00 +/- 1006.45 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 973496.00 +/- 986.66", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9611 +/- 0.0019 ", "");
...
hgcerLegend->AddEntry(h_hgcer_should, "Integral = 919310.00 +/- 958.81","l"); // hgcer should

hgcerLegend->AddEntry(h_hgcer_did,"Did","l");
hgcerLegend->AddEntry(h_hgcer_did,"Integral = 896275.00 +/- 946.72", "l"); // hgcer did

hgcerLegend->AddEntry( h_hgcer_did,"Efficiency = 0.9749 +/- 0.0020", "");
...
ngcerLegend->AddEntry(h_ngcer_should, "Integral = 959689.00 +/- 979.64","l"); // ngcer should

ngcerLegend->AddEntry(h_ngcer_did,"Did","l");
ngcerLegend->AddEntry(h_ngcer_did,"Integral = 913076.00 +/- 955.55", "l"); // ngcer did

ngcerLegend->AddEntry( h_ngcer_did,"Efficiency = 0.9514 +/- 0.0020", "");
...
aeroLegend->AddEntry(h_aero_should, "Integral = 919310.00 +/- 958.81","l"); // aero should

aeroLegend->AddEntry(h_aero_did,"Did","l");
aeroLegend->AddEntry(h_aero_did,"Integral = 896275.00 +/- 946.72", "l"); // aero did

aeroLegend->AddEntry( h_aero_did,"Efficiency = 0.9749 +/- 0.0020", "");
...
c1->Print("16762_hgcer_Eff.pdf","pdf");
c2->Print("16762_cal_Eff.pdf","pdf");
c3->Print("16762_ngcer_Eff.pdf","pdf");
c4->Print("16762_aero_Eff.pdf", "pdf");
}