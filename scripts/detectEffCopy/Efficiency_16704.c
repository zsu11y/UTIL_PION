...
void Efficiency_16704()
{

// Run 16704 - HMS only, LH2, 60 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16704_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS LH2 60uA Efficiency - Run 16704", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS LH2 60uA Efficiency - Run 16704", 300, 0.1, 2);
...
{   // cerenkov fill
    ...
    {
        if(abs(beta[i]-betaCut1)<betaCut2 && caltr[i]>caltrCut && HgtrDP[i] > H_del_low && HgtrDP[i] < H_del_high)
        {                                       // ^^^"should" cuts ^^^
...
{ 
    if(line.find("run 16704 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16704 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16704 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16704 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
calLegend->AddEntry(h_cal_should, "Integral = 507323.00 +/- 712.27 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 505786.00 +/- 711.19 ", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9970 +/- 0.0028 ", "");
...
cerLegend->AddEntry(h_cer_should, "Integral =  507388.00 +/- 712.31","l"); // cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 506061.00 +/- 711.38", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9974 +/- 0.0028 ", "");
...
c1->Print("16704_cer_Eff.pdf","pdf");
c2->Print("16704_cal_Eff.pdf","pdf");
}