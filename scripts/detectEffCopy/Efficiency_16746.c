...
void Efficiency_16746()
{

// Run 16746 - SHMS only, Carbon, 80 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16746_-1.root"); //open a new file for each beam current 
...
// hg cerenkov hists
auto h_hgcer_should = new TH1D ("P.hgcer.npeSum", "SHMS Carbon 80uA Efficiency - Run 16746", 300 , 0.1, 40);
...
// ng cerenkov hists
auto h_ngcer_should = new TH1D ("P.ngcer.npeSum", "SHMS Carbon 80uA Efficiency - Run 16746", 300 , 0.1, 40);
...
// aerogel hists
auto h_aero_should = new TH1D ("P.aero.npeSum","SHMS Carbon 80uA Efficiency - Run 16746", 300, 0.1, 50);
...
// calorimeter hists
auto h_cal_should = new TH1D ("P.cal.etottracknorm", "SHMS Carbon 80uA Efficiency - Run 16746", 300, 0.1, 2);
...
auto bcmAvgCut1 = 80; // run replay lumi to figure out cut values for this run
...
{ 
    if(line.find("run 16746 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16746 - hgcer: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
    if(line.find("run 16746 - ngcer:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16746 - aero: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
}  
...
{
    eff_file_out << "run 16746 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16746 - hgcer: "<< fixed << setprecision(5) << effhgcer << " +/- "  << setprecision(5)<< dEffhgcer << endl;
    eff_file_out << "run 16746 - ngcer: "<< fixed << setprecision(5) << effngcer << " +/- "  << setprecision(5)<< dEffngcer << endl;
    eff_file_out << "run 16746 - aero: "<< fixed << setprecision(5) << effaero << " +/- "  << setprecision(5)<< dEffaero << endl << endl;
    foundhgcer = true;
    foundngcer = true;
    foundcal = true;
    foundaero = true;
...
calLegend->AddEntry(h_cal_should, "Integral = 848986.00 +/- 921.40 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 819898.00 +/- 905.48", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9657 +/- 0.0021 ", "");
...
hgcerLegend->AddEntry(h_hgcer_should, "Integral = 773361.00 +/- 879.41","l"); // hgcer should

hgcerLegend->AddEntry(h_hgcer_did,"Did","l");
hgcerLegend->AddEntry(h_hgcer_did,"Integral = 751967.00 +/- 867.16", "l"); // hgcer did

hgcerLegend->AddEntry( h_hgcer_did,"Efficiency = 0.9723 +/- 0.0022", "");
...
ngcerLegend->AddEntry(h_ngcer_should, "Integral = 807912.00 +/- 898.84","l"); // ngcer should

ngcerLegend->AddEntry(h_ngcer_did,"Did","l");
ngcerLegend->AddEntry(h_ngcer_did,"Integral = 765975.00 +/- 875.20", "l"); // ngcer did

ngcerLegend->AddEntry( h_ngcer_did,"Efficiency = 0.9481 +/- 0.0021", "");
...
aeroLegend->AddEntry(h_aero_should, "Integral = 773361.00 +/- 879.41","l"); // aero should

aeroLegend->AddEntry(h_aero_did,"Did","l");
aeroLegend->AddEntry(h_aero_did,"Integral = 751967.00 +/- 867.16", "l"); // aero did

aeroLegend->AddEntry( h_aero_did,"Efficiency = 0.9723 +/- 0.0022 ", "");
...
c1->Print("16746_hgcer_Eff.pdf","pdf");
c2->Print("16746_cal_Eff.pdf","pdf");
c3->Print("16746_ngcer_Eff.pdf","pdf");
c4->Print("16746_aero_Eff.pdf", "pdf");
}