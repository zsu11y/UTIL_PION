...
void Efficiency_16735()
{

// Run 16735 - HMS only, Carbon, 5 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16735_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS Carbon 5uA Efficiency - Run 16735", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS Carbon 5uA Efficiency - Run 16735", 300, 0.1, 2);
...
{ 
    if(line.find("run 16735 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16735 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16735 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16735 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
calLegend->AddEntry(h_cal_should, "Integral = 880258.00 +/- 938.22 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 878118.00 +/- 937.08 ", "l"); // cal did 
...
cerLegend->AddEntry(h_cer_should, "Integral = 878896.00 +/- 937.49","l"); // cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 877994.00 +/- 937.01", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9990 +/- 0.0021", "");
...
c1->Print("16735_cer_Eff.pdf","pdf");
c2->Print("16735_cal_Eff.pdf","pdf");
}