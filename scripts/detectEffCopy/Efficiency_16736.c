...
void Efficiency_16736()
{

// Run 16736 - HMS only, Carbon, 2.5 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16736_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS Carbon 2.5uA Efficiency - Run 16736", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS Carbon 2.5uA Efficiency - Run 16736", 300, 0.1, 2);
...
{ 
    if(line.find("run 16736 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16736 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16736 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16736 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
calLegend->AddEntry(h_cal_should, "Integral = 675501.00 +/- 821.89 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 673896.00 +/- 820.91 ", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9976 +/- 0.0024", "");
...
cerLegend->AddEntry(h_cer_should, "Integral = 674567.00 +/- 821.32","l"); // cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 673757.00 +/- 820.83", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9988 +/- 0.0024", "");
...
c1->Print("16736_cer_Eff.pdf","pdf");
c2->Print("16736_cal_Eff.pdf","pdf");
}