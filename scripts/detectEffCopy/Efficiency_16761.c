...
void Efficiency_16761()
{

// Run 16761 - SHMS only, LH2, 25 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16761_-1.root"); //open a new file for each beam current 
...
// hg cerenkov hists
auto h_hgcer_should = new TH1D ("P.hgcer.npeSum", "SHMS LH2 25uA Efficiency - Run 16761", 300 , 0.1, 40);
...
// ng cerenkov hists
auto h_ngcer_should = new TH1D ("P.ngcer.npeSum", "SHMS LH2 25uA Efficiency - Run 16761", 300 , 0.1, 40);
...
// aerogel hists
auto h_aero_should = new TH1D ("P.aero.npeSum","SHMS LH2 25uA Efficiency - Run 16761", 300, 0.1, 50);
...
// calorimeter hists
auto h_cal_should = new TH1D ("P.cal.etottracknorm", "SHMS LH2 25uA Efficiency - Run 16761", 300, 0.1, 2);
...
{ 
    if(line.find("run 16761 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16761 - hgcer: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
    if(line.find("run 16761 - ngcer:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16761 - aero: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
}  
...
{
    eff_file_out << "run 16761 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16761 - hgcer: "<< fixed << setprecision(5) << effhgcer << " +/- "  << setprecision(5)<< dEffhgcer << endl;
    eff_file_out << "run 16761 - ngcer: "<< fixed << setprecision(5) << effngcer << " +/- "  << setprecision(5)<< dEffngcer << endl;
    eff_file_out << "run 16761 - aero: "<< fixed << setprecision(5) << effaero << " +/- "  << setprecision(5)<< dEffaero << endl << endl;
    foundhgcer = true;
    foundngcer = true;
    foundcal = true;
    foundaero = true;
...
calLegend->AddEntry(h_cal_should, "Integral = 685684.00 +/- 828.06 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 658712.00 +/- 811.61", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9607 +/- 0.0023 ", "");
...
hgcerLegend->AddEntry(h_hgcer_should, "Integral = 620630.00 +/- 787.80","l"); // hgcer should

hgcerLegend->AddEntry(h_hgcer_did,"Did","l");
hgcerLegend->AddEntry(h_hgcer_did,"Integral = 604671.00 +/- 777.61", "l"); // hgcer did

hgcerLegend->AddEntry( h_hgcer_did,"Efficiency = 0.9743 +/- 0.0025", "");
...
ngcerLegend->AddEntry(h_ngcer_should, "Integral = 648325.00 +/- 805.19","l"); // ngcer should

ngcerLegend->AddEntry(h_ngcer_did,"Did","l");
ngcerLegend->AddEntry(h_ngcer_did,"Integral = 616571.00 +/- 785.22", "l"); // ngcer did

ngcerLegend->AddEntry( h_ngcer_did,"Efficiency = 0.9510 +/- 0.0024", "");
...
aeroLegend->AddEntry(h_aero_should, "Integral = 620630.00 +/- 787.80","l"); // aero should

aeroLegend->AddEntry(h_aero_did,"Did","l");
aeroLegend->AddEntry(h_aero_did,"Integral = 604671.00 +/- 777.61", "l"); // aero did

aeroLegend->AddEntry( h_aero_did,"Efficiency = 0.9743 +/- 0.0025 ", "");
...
c1->Print("16761_hgcer_Eff.pdf","pdf");
c2->Print("16761_cal_Eff.pdf","pdf");
c3->Print("16761_ngcer_Eff.pdf","pdf");
c4->Print("16761_aero_Eff.pdf", "pdf");
}