...
void Efficiency_16712()
{

// Run 16712 - HMS only, LH2, 2.5 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16712_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS LH2 2.5uA Efficiency - Run 16712", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS LH2 2.5uA Efficiency - Run 16712", 300, 0.1, 2);
...
{ 
    if(line.find("run 16712 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16712 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16712 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16712 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
calLegend->AddEntry(h_cal_should, "Integral = 79344.00 +/- 281.68 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 79182.00 +/- 281.39 ", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9980 +/- 0.0071 ", "");
...
cerLegend->AddEntry(h_cer_should, "Integral = 79269.00 +/- 281.55","l"); // cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 79171.00 +/- 281.37", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9988 +/- 0.0071", "");
...
c1->Print("16712_cer_Eff.pdf","pdf");
c2->Print("16712_cal_Eff.pdf","pdf");
}