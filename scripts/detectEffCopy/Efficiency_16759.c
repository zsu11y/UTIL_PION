...
void Efficiency_16759()
{

// Run 16759 - SHMS only, LH2, 80 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16759_-1.root"); //open a new file for each beam current 
...
// hg cerenkov hists
auto h_hgcer_should = new TH1D ("P.hgcer.npeSum", "SHMS LH2 80uA Efficiency - Run 16759", 300 , 0.1, 40);
...
// ng cerenkov hists
auto h_ngcer_should = new TH1D ("P.ngcer.npeSum", "SHMS LH2 80uA Efficiency - Run 16759", 300 , 0.1, 40);
...
// aerogel hists
auto h_aero_should = new TH1D ("P.aero.npeSum","SHMS LH2 80uA Efficiency - Run 16759", 300, 0.1, 50);
...
// calorimeter hists
auto h_cal_should = new TH1D ("P.cal.etottracknorm", "SHMS LH2 80uA Efficiency - Run 16759", 300, 0.1, 2);
...
{ 
    if(line.find("run 16759 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16759 - hgcer: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
    if(line.find("run 16759 - ngcer:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16759 - aero: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
}  
...
{
    eff_file_out << "run 16759 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16759 - hgcer: "<< fixed << setprecision(5) << effhgcer << " +/- "  << setprecision(5)<< dEffhgcer << endl;
    eff_file_out << "run 16759 - ngcer: "<< fixed << setprecision(5) << effngcer << " +/- "  << setprecision(5)<< dEffngcer << endl;
    eff_file_out << "run 16759 - aero: "<< fixed << setprecision(5) << effaero << " +/- "  << setprecision(5)<< dEffaero << endl << endl;
    foundhgcer = true;
    foundngcer = true;
    foundcal = true;
    foundaero = true;
...
calLegend->AddEntry(h_cal_should, "Integral = 461590.00 +/- 679.40 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 442931.00 +/- 665.53", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9596 +/- 0.0029 ", "");
...
hgcerLegend->AddEntry(h_hgcer_should, "Integral = 412097.00 +/- 641.95","l"); // hgcer should

hgcerLegend->AddEntry(h_hgcer_did,"Did","l");
hgcerLegend->AddEntry(h_hgcer_did,"Integral = 401738.00 +/- 633.83", "l"); // hgcer did

hgcerLegend->AddEntry( h_hgcer_did,"Efficiency = 0.9749 +/- 0.0031", "");
...
ngcerLegend->AddEntry(h_ngcer_should, "Integral = 433116.00 +/- 658.12","l"); // ngcer should

ngcerLegend->AddEntry(h_ngcer_did,"Did","l");
ngcerLegend->AddEntry(h_ngcer_did,"Integral = 412140.00 +/- 641.98", "l"); // ngcer did

ngcerLegend->AddEntry( h_ngcer_did,"Efficiency = 0.9516 +/- 0.0029", "");
...
aeroLegend->AddEntry(h_aero_should, "Integral = 412097.00 +/- 641.95","l"); // aero should

aeroLegend->AddEntry(h_aero_did,"Did","l");
aeroLegend->AddEntry(h_aero_did,"Integral = 401738.00 +/- 633.83", "l"); // aero did

aeroLegend->AddEntry( h_aero_did,"Efficiency = 0.9749 +/- 0.0031 ", "");
...
c1->Print("16759_hgcer_Eff.pdf","pdf");
c2->Print("16759_cal_Eff.pdf","pdf");
c3->Print("16759_ngcer_Eff.pdf","pdf");
c4->Print("16759_aero_Eff.pdf", "pdf");
}