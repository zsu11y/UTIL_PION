...
void Efficiency_16703()
{

// Run 16703 - HMS only, LH2, 80 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16703_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS LH2 80uA Efficiency - Run 16703", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS LH2 80uA Efficiency - Run 16703", 300, 0.1, 2);
...
{ 
    if(line.find("run 16703 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16703 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16703 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16703 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
calLegend->AddEntry(h_cal_should, "Integral = 757071.00 +/- 870.10 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 754426.00 +/- 868.58 ", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9965 +/- 0.0023", "");
...
cerLegend->AddEntry(h_cer_should, "Integral =  757242.00 +/- 870.20","l"); // cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 755077.00 +/- 868.95", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9971 +/- 0.0023", "");
...
c1->Print("16703_cer_Eff.pdf","pdf");
c2->Print("16703_cal_Eff.pdf","pdf");
}