...
void Efficiency_16742()
{

// Run 16742 - SHMS only, Carbon, 15 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16742_-1.root"); //open a new file for each beam current 
...
// hg cerenkov hists
auto h_hgcer_should = new TH1D ("P.hgcer.npeSum", "SHMS Carbon 15uA Efficiency - Run 16742", 300 , 0.1, 40);
...
// ng cerenkov hists
auto h_ngcer_should = new TH1D ("P.ngcer.npeSum", "SHMS Carbon 15uA Efficiency - Run 16742", 300 , 0.1, 40);
...
// aerogel hists
auto h_aero_should = new TH1D ("P.aero.npeSum","SHMS Carbon 15uA Efficiency - Run 16742", 300, 0.1, 50);
...
// calorimeter hists
auto h_cal_should = new TH1D ("P.cal.etottracknorm", "SHMS Carbon 15uA Efficiency - Run 16742", 300, 0.1, 2);
...
auto bcmAvgCut1 = 15; // run replay lumi to figure out cut values for this run
auto bcmAvgCut2 = 4;
...
{ 
    if(line.find("run 16742 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16742 - hgcer: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
    if(line.find("run 16742 - ngcer:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16742 - aero: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
}  
...
{
    eff_file_out << "run 16742 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16742 - hgcer: "<< fixed << setprecision(5) << effhgcer << " +/- "  << setprecision(5)<< dEffhgcer << endl;
    eff_file_out << "run 16742 - ngcer: "<< fixed << setprecision(5) << effngcer << " +/- "  << setprecision(5)<< dEffngcer << endl;
    eff_file_out << "run 16742 - aero: "<< fixed << setprecision(5) << effaero << " +/- "  << setprecision(5)<< dEffaero << endl << endl;
    foundhgcer = true;
    foundngcer = true;
    foundcal = true;
    foundaero = true;
...
calLegend->AddEntry(h_cal_should, "Integral = 662164.00 +/- 813.73 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 639901.00 +/- 799.94", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9664 +/- 0.0024 ", "");
...
hgcerLegend->AddEntry(h_hgcer_should, "Integral = 605553.00 +/- 778.17","l"); // hgcer should

hgcerLegend->AddEntry(h_hgcer_did,"Did","l");
hgcerLegend->AddEntry(h_hgcer_did,"Integral = 588864.00 +/- 767.37", "l"); // hgcer did

hgcerLegend->AddEntry( h_hgcer_did,"Efficiency = 0.9724 +/- 0.0025", "");
...
ngcerLegend->AddEntry(h_ngcer_should, "Integral = 631394.00 +/- 794.60","l"); // ngcer should

ngcerLegend->AddEntry(h_ngcer_did,"Did","l");
ngcerLegend->AddEntry(h_ngcer_did,"Integral = 598807.00 +/- 773.83", "l"); // ngcer did

ngcerLegend->AddEntry( h_ngcer_did,"Efficiency = 0.9484 +/- 0.0024", "");
...
aeroLegend->AddEntry(h_aero_should, "Integral = 605553.00 +/- 778.17","l"); // aero should

aeroLegend->AddEntry(h_aero_did,"Did","l");
aeroLegend->AddEntry(h_aero_did,"Integral = 588864.00 +/- 767.37", "l"); // aero did

aeroLegend->AddEntry( h_aero_did,"Efficiency = 0.9724 +/- 0.0025 ", "");
...
c1->Print("16742_hgcer_Eff.pdf","pdf");
c2->Print("16742_cal_Eff.pdf","pdf");
c3->Print("16742_ngcer_Eff.pdf","pdf");
c4->Print("16742_aero_Eff.pdf", "pdf");
}