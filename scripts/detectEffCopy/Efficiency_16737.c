...
void Efficiency_16737()
{

// Run 16737 - HMS only, Carbon, 1 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16737_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS Carbon 1uA Efficiency - Run 16737", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS Carbon 1uA Efficiency - Run 16737", 300, 0.1, 2);
...
{ 
    if(line.find("run 16737 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16737 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16737 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16737 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
calLegend->AddEntry(h_cal_should, "Integral = 245034.00 +/- 495.01 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 244457.00 +/- 494.43 ", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9976 +/- 0.0040", "");
...
cerLegend->AddEntry(h_cer_should, "Integral = 244697.00 +/- 494.67","l"); // cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 244406.00 +/- 494.37", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9988 +/- 0.0040", "");
...
c1->Print("16737_cer_Eff.pdf","pdf");
c2->Print("16737_cal_Eff.pdf","pdf");
}