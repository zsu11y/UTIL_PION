...
void Efficiency_16728()
{

// Run 16728 - HMS only, Carbon, 80 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16728_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS Carbon 80uA Efficiency - Run 16728", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS Carbon 80uA Efficiency - Run 16728", 300, 0.1, 2);
...
printf(" 16728_cal Integral Should = %0.2f +/- %0.2f\n", Scal, dScal);

double dDcal= 0.0;
double Dcal = h_cal_did->IntegralAndError(1,300,dDcal,"");	
printf(" 16728_cal Integral Did = %0.2f +/- %0.2f\n",Dcal,dDcal);

double effcal = (Dcal/Scal);
double dEffcal = sqrt((effcal*(1-effcal))/Scal);
printf(" 16728_cal Efficiency = %0.5f +/- %0.5f \n\n", effcal,dEffcal);

double dScer = 0.0;
double Scer = h_cer_should->IntegralAndError(1,300,dScer,""); // "should" integral
printf(" 16728_cer Integral Should = %0.2f +/- %0.2f\n", Scer, dScer); 

double dDcer= 0.0;
double Dcer = h_cer_did->IntegralAndError(1,300,dDcer,"");  // "did" integral
printf(" 16728_cer Integral Did = %0.2f +/- %0.2f\n",Dcer,dDcer);

double effcer = (Dcer/Scer);    
double dEffcer = sqrt((effcer*(1-effcer))/Scer);                        // efficiency == did/should 
printf(" 16728_cer Efficiency = %0.5f +/- %0.5f \n\n", effcer, dEffcer);
...
{ 
    if(line.find("run 16728 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16728 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16728 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16728 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
calLegend->AddEntry(h_cal_should, "Integral = 842870.00 +/- 918.08 ","l"); //cal should

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 840708.00 +/- 916.90 ", "l"); // cal did

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9974 +/- 0.0022", "");
calLegend->SetTextSize(0.028);

auto cerLegend = new TLegend(0.55,0.5,0.9,0.75); // manually entered the integral and efficiency values from terminal output
cerLegend->AddEntry(h_cer_should,"Should","l");
cerLegend->AddEntry(h_cer_should, "Integral = 841942.00 +/- 917.57","l"); //cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 840758.00 +/- 916.93", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9986 +/- 0.0022", "");
...
c1->Print("16728_cer_Eff.pdf","pdf");
c2->Print("16728_cal_Eff.pdf","pdf");
