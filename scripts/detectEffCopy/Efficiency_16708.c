...
void Efficiency_16708()
{

// Run 16708 - HMS only, LH2, 25 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16708_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS LH2 25uA Efficiency - Run 16708", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS LH2 25uA Efficiency - Run 16708", 300, 0.1, 2);
...
{ 
    if(line.find("run 16708 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16708 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16708 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16708 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
calLegend->AddEntry(h_cal_should, "Integral = 796873.00 +/- 892.68 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 794657.00 +/- 891.44 ", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9972 +/- 0.0022 ", "");
...
cerLegend->AddEntry(h_cer_should, "Integral = 796146.00 +/- 892.27","l"); // cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 794812.00 +/- 891.52", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9983 +/- 0.0022", "");
...
c1->Print("16708_cer_Eff.pdf","pdf");
c2->Print("16708_cal_Eff.pdf","pdf");
}