...
void Efficiency_16710()
{

// Run 16710 - HMS only, LH2, 8 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16710_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS LH2 8uA Efficiency - Run 16710", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS LH2 8uA Efficiency - Run 16710", 300, 0.1, 2);
...
{   // cerenkov fill
    ...
    {
        if(abs(beta[i]-betaCut1)<betaCut2 && cerNpeSum[i] > cerNpeSumCut && HgtrDP[i] > H_del_low && HgtrDP[i] < H_del_high)
        {                                   // ^^^ "should" cuts ^^^
...
{ 
    if(line.find("run 16710 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16710 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16710 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16710 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
calLegend->AddEntry(h_cal_should, "Integral = 589618.00 +/- 767.87 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 588102.00 +/- 766.88 ", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9974 +/- 0.0026 ", "");
...
cerLegend->AddEntry(h_cer_should, "Integral = 588839.00 +/- 767.36","l"); // cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 588064.00 +/- 766.85", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9987 +/- 0.0026", "");
...
c1->Print("16710_cer_Eff.pdf","pdf");
c2->Print("16710_cal_Eff.pdf","pdf");
}