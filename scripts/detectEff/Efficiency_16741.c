...
void Efficiency_16741()
{

// Run 16741 - SHMS only, Carbon, 8 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16741_-1.root"); //open a new file for each beam current 
...
// hg cerenkov hists
auto h_hgcer_should = new TH1D ("P.hgcer.npeSum", "SHMS Carbon 8uA Efficiency - Run 16741", 300 , 0.1, 40);
...
// ng cerenkov hists
auto h_ngcer_should = new TH1D ("P.ngcer.npeSum", "SHMS Carbon 8uA Efficiency - Run 16741", 300 , 0.1, 40);
...
// aerogel hists
auto h_aero_should = new TH1D ("P.aero.npeSum","SHMS Carbon 8uA Efficiency - Run 16741", 300, 0.1, 50);
...
// calorimeter hists
auto h_cal_should = new TH1D ("P.cal.etottracknorm", "SHMS Carbon 8uA Efficiency - Run 16741", 300, 0.1, 2);
...
auto bcmAvgCut1 = 8; // run replay lumi to figure out cut values for this run
auto bcmAvgCut2 = 3;
...
{ 
    if(line.find("run 16741 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16741 - hgcer: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
    if(line.find("run 16741 - ngcer:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16741 - aero: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
}  
...
{
    eff_file_out << "run 16741 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16741 - hgcer: "<< fixed << setprecision(5) << effhgcer << " +/- "  << setprecision(5)<< dEffhgcer << endl;
    eff_file_out << "run 16741 - ngcer: "<< fixed << setprecision(5) << effngcer << " +/- "  << setprecision(5)<< dEffngcer << endl;
    eff_file_out << "run 16741 - aero: "<< fixed << setprecision(5) << effaero << " +/- "  << setprecision(5)<< dEffaero << endl << endl;
    foundhgcer = true;
    foundngcer = true;
    foundcal = true;
    foundaero = true;
...
calLegend->AddEntry(h_cal_should, "Integral = 734615.00 +/- 857.10 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 710113.00 +/- 842.68", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9666 +/- 0.0023 ", "");
...
hgcerLegend->AddEntry(h_hgcer_should, "Integral = 671002.00 +/- 819.15","l"); // hgcer should

hgcerLegend->AddEntry(h_hgcer_did,"Did","l");
hgcerLegend->AddEntry(h_hgcer_did,"Integral = 652786.00 +/- 807.95", "l"); // hgcer did

hgcerLegend->AddEntry( h_hgcer_did,"Efficiency = 0.9729 +/- 0.0024", "");
...
ngcerLegend->AddEntry(h_ngcer_should, "Integral = 700514.00 +/- 836.97","l"); // ngcer should

ngcerLegend->AddEntry(h_ngcer_did,"Did","l");
ngcerLegend->AddEntry(h_ngcer_did,"Integral = 663638.00 +/- 814.64", "l"); // ngcer did

ngcerLegend->AddEntry( h_ngcer_did,"Efficiency = 0.9474 +/- 0.0023", "");
...
aeroLegend->AddEntry(h_aero_should, "Integral = 671002.00 +/- 819.15","l"); // aero should

aeroLegend->AddEntry(h_aero_did,"Did","l");
aeroLegend->AddEntry(h_aero_did,"Integral = 652786.00 +/- 807.95", "l"); // aero did

aeroLegend->AddEntry( h_aero_did,"Efficiency = 0.9729 +/- 0.0024 ", "");
...
c1->Print("16741_hgcer_Eff.pdf","pdf");
c2->Print("16741_cal_Eff.pdf","pdf");
c3->Print("16741_ngcer_Eff.pdf","pdf");
c4->Print("16741_aero_Eff.pdf", "pdf");
}