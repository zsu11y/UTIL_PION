...
void Efficiency_16744()
{

// Run 16744 - SHMS only, Carbon, 40 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16744_-1.root"); //open a new file for each beam current 
...
// hg cerenkov hists
auto h_hgcer_should = new TH1D ("P.hgcer.npeSum", "SHMS Carbon 40uA Efficiency - Run 16744", 300 , 0.1, 40);
...
// ng cerenkov hists
auto h_ngcer_should = new TH1D ("P.ngcer.npeSum", "SHMS Carbon 40uA Efficiency - Run 16744", 300 , 0.1, 40);
...
// aerogel hists
auto h_aero_should = new TH1D ("P.aero.npeSum","SHMS Carbon 40uA Efficiency - Run 16744", 300, 0.1, 50);
...
// calorimeter hists
auto h_cal_should = new TH1D ("P.cal.etottracknorm", "SHMS Carbon 40uA Efficiency - Run 16744", 300, 0.1, 2);
...
auto bcmAvgCut1 = 40; // run replay lumi to figure out cut values for this run
auto bcmAvgCut2 = 6.0;
...
{ 
    if(line.find("run 16744 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16744 - hgcer: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
    if(line.find("run 16744 - ngcer:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16744 - aero: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
}  
...
{
    eff_file_out << "run 16744 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16744 - hgcer: "<< fixed << setprecision(5) << effhgcer << " +/- "  << setprecision(5)<< dEffhgcer << endl;
    eff_file_out << "run 16744 - ngcer: "<< fixed << setprecision(5) << effngcer << " +/- "  << setprecision(5)<< dEffngcer << endl;
    eff_file_out << "run 16744 - aero: "<< fixed << setprecision(5) << effaero << " +/- "  << setprecision(5)<< dEffaero << endl << endl;
    foundhgcer = true;
    foundngcer = true;
    foundcal = true;
    foundaero = true;
...
calLegend->AddEntry(h_cal_should, "Integral = 667985.00 +/- 817.307 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 644946.00 +/- 803.09", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9655 +/- 0.0024 ", "");
...
hgcerLegend->AddEntry(h_hgcer_should, "Integral = 608949.00 +/- 780.35","l"); // hgcer should

hgcerLegend->AddEntry(h_hgcer_did,"Did","l");
hgcerLegend->AddEntry(h_hgcer_did,"Integral = 592180.00 +/- 769.53", "l"); // hgcer did

hgcerLegend->AddEntry( h_hgcer_did,"Efficiency = 0.9725 +/- 0.0025", "");
...
ngcerLegend->AddEntry(h_ngcer_should, "Integral = 635984.00 +/- 797.49","l"); // ngcer should

ngcerLegend->AddEntry(h_ngcer_did,"Did","l");
ngcerLegend->AddEntry(h_ngcer_did,"Integral = 602572.00 +/- 776.26", "l"); // ngcer did

ngcerLegend->AddEntry( h_ngcer_did,"Efficiency = 0.9475 +/- 0.0024", "");
...
aeroLegend->AddEntry(h_aero_should, "Integral = 608949.00 +/- 780.35","l"); // aero should

aeroLegend->AddEntry(h_aero_did,"Did","l");
aeroLegend->AddEntry(h_aero_did,"Integral = 592180.00 +/- 769.53", "l"); // aero did

aeroLegend->AddEntry( h_aero_did,"Efficiency = 0.9725 +/- 0.0025 ", "");
...
c1->Print("16744_hgcer_Eff.pdf","pdf");
c2->Print("16744_cal_Eff.pdf","pdf");
c3->Print("16744_ngcer_Eff.pdf","pdf");
c4->Print("16744_aero_Eff.pdf", "pdf");
}