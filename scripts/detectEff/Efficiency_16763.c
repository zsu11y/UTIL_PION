...
void Efficiency_16763()
{

// Run 16763 - SHMS only, LH2, 8 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16763_-1.root"); //open a new file for each beam current 
...
// hg cerenkov hists
auto h_hgcer_should = new TH1D ("P.hgcer.npeSum", "SHMS LH2 8uA Efficiency - Run 16763", 300 , 0.1, 40);
...
// ng cerenkov hists
auto h_ngcer_should = new TH1D ("P.ngcer.npeSum", "SHMS LH2 8uA Efficiency - Run 16763", 300 , 0.1, 40);
...
// aerogel hists
auto h_aero_should = new TH1D ("P.aero.npeSum","SHMS LH2 8uA Efficiency - Run 16763", 300, 0.1, 50);
...
// calorimeter hists
auto h_cal_should = new TH1D ("P.cal.etottracknorm", "SHMS LH2 8uA Efficiency - Run 16763", 300, 0.1, 2);
...
{ 
    if(line.find("run 16763 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16763 - hgcer: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
    if(line.find("run 16763 - ngcer:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16763 - aero: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
}  
...
{
    eff_file_out << "run 16763 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16763 - hgcer: "<< fixed << setprecision(5) << effhgcer << " +/- "  << setprecision(5)<< dEffhgcer << endl;
    eff_file_out << "run 16763 - ngcer: "<< fixed << setprecision(5) << effngcer << " +/- "  << setprecision(5)<< dEffngcer << endl;
    eff_file_out << "run 16763 - aero: "<< fixed << setprecision(5) << effaero << " +/- "  << setprecision(5)<< dEffaero << endl << endl;
    foundhgcer = true;
    foundngcer = true;
    foundcal = true;
    foundaero = true;
...
calLegend->AddEntry(h_cal_should, "Integral = 507930.00 +/- 712.69 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 488161.00 +/- 698.69", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9611 +/- 0.0027 ", "");
...
hgcerLegend->AddEntry(h_hgcer_should, "Integral = 461333.00 +/- 679.21","l"); // hgcer should

hgcerLegend->AddEntry(h_hgcer_did,"Did","l");
hgcerLegend->AddEntry(h_hgcer_did,"Integral = 449830.00 +/- 670.69", "l"); // hgcer did

hgcerLegend->AddEntry( h_hgcer_did,"Efficiency = 0.9751 +/- 0.0029", "");
...
ngcerLegend->AddEntry(h_ngcer_should, "Integral = 481598.00 +/- 693.97","l"); // ngcer should

ngcerLegend->AddEntry(h_ngcer_did,"Did","l");
ngcerLegend->AddEntry(h_ngcer_did,"Integral = 457905.00 +/- 676.69", "l"); // ngcer did

ngcerLegend->AddEntry( h_ngcer_did,"Efficiency = 0.9508 +/- 0.0028", "");
...
aeroLegend->AddEntry(h_aero_should, "Integral = 461333.00 +/- 679.21","l"); // aero should

aeroLegend->AddEntry(h_aero_did,"Did","l");
aeroLegend->AddEntry(h_aero_did,"Integral = 449830.00 +/- 670.69", "l"); // aero did

aeroLegend->AddEntry( h_aero_did,"Efficiency = 0.9751 +/- 0.0029", "");
...
c1->Print("16763_hgcer_Eff.pdf","pdf");
c2->Print("16763_cal_Eff.pdf","pdf");
c3->Print("16763_ngcer_Eff.pdf","pdf");
c4->Print("16763_aero_Eff.pdf", "pdf");
}