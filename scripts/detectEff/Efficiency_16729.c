...
void Efficiency_16729()
{

// Run 16729 - HMS only, Carbon, 60 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16729_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS Carbon 60uA Efficiency - Run 16729", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS Carbon 60uA Efficiency - Run 16729", 300, 0.1, 2);
...
{ 
    if(line.find("run 16729 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16729 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16729 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16729 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
calLegend->AddEntry(h_cal_should, "Integral = 958616.00 +/- 979.09 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 956153.00 +/- 977.83 ", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9974 +/- 0.0020", "");
...
cerLegend->AddEntry(h_cer_should, "Integral = 957695.00 +/- 978.62","l"); // cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 956153.00 +/- 977.83", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9983 +/- 0.0020", "");
...
c1->Print("16729_cer_Eff.pdf","pdf");
c2->Print("16729_cal_Eff.pdf","pdf");
}