...
void Efficiency_16730()
{

// Run 16730 - HMS only, Carbon, 40 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16730_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS Carbon 40uA Efficiency - Run 16730", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS Carbon 40uA Efficiency - Run 16730", 300, 0.1, 2);
...
{ 
    if(line.find("run 16730 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16730 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16730 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16730 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
cerLegend->AddEntry(h_cer_should, "Integral = 864387.00 +/- 929.72","l"); // cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 863267.00 +/- 929.12", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9987 +/- 0.0021", "");
cerLegend->SetTextSize(0.028);

auto calLegend = new TLegend(0.55,0.5,0.9,0.75);
calLegend->AddEntry(h_cal_should,"Should","l");
calLegend->AddEntry(h_cal_should, "Integral = 865354.00 +/- 930.24 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 863288.00 +/- 929.13 ", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9976 +/- 0.0021", "");
...
c1->Print("16730_cer_Eff.pdf","pdf");
c2->Print("16730_cal_Eff.pdf","pdf");
}