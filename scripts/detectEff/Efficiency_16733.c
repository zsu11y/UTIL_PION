...
void Efficiency_16733()
{

// Run 16733 - HMS only, Carbon, 15 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16733_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS Carbon 15uA Efficiency - Run 16733", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS Carbon 15uA Efficiency - Run 16733", 300, 0.1, 2);
...
{ 
    if(line.find("run 16733 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16733 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16733 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16733 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
calLegend->AddEntry(h_cal_should, "Integral = 731611.00 +/- 855.34 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 729822.00 +/- 854.30 ", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9976 +/- 0.0023", "");
...
cerLegend->AddEntry(h_cer_should, "Integral = 734239.00 +/- 856.88","l"); // cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 729740.00 +/- 854.25", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9989 +/- 0.0023", "");
...
c1->Print("16733_cer_Eff.pdf","pdf");
c2->Print("16733_cal_Eff.pdf","pdf");
}