...
void Efficiency_16740()
{

// Run 16740 - SHMS only, Carbon, 5 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16740_-1.root"); //open a new file for each beam current 
...
// hg cerenkov hists
auto h_hgcer_should = new TH1D ("P.hgcer.npeSum", "SHMS Carbon 5uA Efficiency - Run 16740", 300 , 0.1, 40);
...
// ng cerenkov hists
auto h_ngcer_should = new TH1D ("P.ngcer.npeSum", "SHMS Carbon 5uA Efficiency - Run 16740", 300 , 0.1, 40);
...
// aerogel hists
auto h_aero_should = new TH1D ("P.aero.npeSum","SHMS Carbon 5uA Efficiency - Run 16740", 300, 0.1, 50);
...
// calorimeter hists
auto h_cal_should = new TH1D ("P.cal.etottracknorm", "SHMS Carbon 5uA Efficiency - Run 16740", 300, 0.1, 2);
...
auto bcmAvgCut1 = 5; // run replay lumi to figure out cut values for this run
auto bcmAvgCut2 = 2;
...
{ 
    if(line.find("run 16740 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16740 - hgcer: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
    if(line.find("run 16740 - ngcer:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16740 - aero: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
}  
...
{
    eff_file_out << "run 16740 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16740 - hgcer: "<< fixed << setprecision(5) << effhgcer << " +/- "  << setprecision(5)<< dEffhgcer << endl;
    eff_file_out << "run 16740 - ngcer: "<< fixed << setprecision(5) << effngcer << " +/- "  << setprecision(5)<< dEffngcer << endl;
    eff_file_out << "run 16740 - aero: "<< fixed << setprecision(5) << effaero << " +/- "  << setprecision(5)<< dEffaero << endl << endl;
    foundhgcer = true;
    foundngcer = true;
    foundcal = true;
    foundaero = true;
...
calLegend->AddEntry(h_cal_should, "Integral = 497221.00 +/- 705.14 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 480856.00 +/- 693.44", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9671 +/- 0.0028", "");
...
hgcerLegend->AddEntry(h_hgcer_should, "Integral = 454253.00 +/- 673.98","l"); // hgcer should

hgcerLegend->AddEntry(h_hgcer_did,"Did","l");
hgcerLegend->AddEntry(h_hgcer_did,"Integral = 441802.00 +/- 664.68", "l"); // hgcer did

hgcerLegend->AddEntry( h_hgcer_did,"Efficiency = 0.9726 +/- 0.0029", "");
...
ngcerLegend->AddEntry(h_ngcer_should, "Integral = 474505.00 +/- 688.84","l"); // ngcer should

ngcerLegend->AddEntry(h_ngcer_did,"Did","l");
ngcerLegend->AddEntry(h_ngcer_did,"Integral = 449171.00 +/- 670.20", "l"); // ngcer did

ngcerLegend->AddEntry( h_ngcer_did,"Efficiency = 0.9466 +/- 0.0028", "");
...
aeroLegend->AddEntry(h_aero_should, "Integral = 454253.00 +/- 673.98","l"); // aero should

aeroLegend->AddEntry(h_aero_did,"Did","l");
aeroLegend->AddEntry(h_aero_did,"Integral = 441802.00 +/- 664.68", "l"); // aero did

aeroLegend->AddEntry( h_aero_did,"Efficiency = 0.9726 +/- 0.0029", "");
...
c1->Print("16740_hgcer_Eff.pdf","pdf");
c2->Print("16740_cal_Eff.pdf","pdf");
c3->Print("16740_ngcer_Eff.pdf","pdf");
c4->Print("16740_aero_Eff.pdf", "pdf");
}