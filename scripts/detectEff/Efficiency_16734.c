...
void Efficiency_16734()
{

// Run 16734 - HMS only, Carbon, 8 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16734_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS Carbon 8uA Efficiency - Run 16734", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS Carbon 8uA Efficiency - Run 16734", 300, 0.1, 2);
...
{ 
    if(line.find("run 16734 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16734 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16734 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16734 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
calLegend->AddEntry(h_cal_should, "Integral = 853217.00 +/- 923.70 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 851257.00 +/- 922.64", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9977 +/- 0.0022", "");
...
cerLegend->AddEntry(h_cer_should, "Integral = 852094.00 +/- 923.09","l"); // cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 851172.00 +/- 922.59", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9989 +/- 0.0022", "");
...
c1->Print("16734_cer_Eff.pdf","pdf");
c2->Print("16734_cal_Eff.pdf","pdf");
}