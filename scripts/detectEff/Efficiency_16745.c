...
void Efficiency_16745()
{

// Run 16745 - SHMS only, Carbon, 60 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16745_-1.root"); //open a new file for each beam current 
...
// hg cerenkov hists
auto h_hgcer_should = new TH1D ("P.hgcer.npeSum", "SHMS Carbon 60uA Efficiency - Run 16745", 300 , 0.1, 40);
...
// ng cerenkov hists
auto h_ngcer_should = new TH1D ("P.ngcer.npeSum", "SHMS Carbon 60uA Efficiency - Run 16745", 300 , 0.1, 40);
...
// aerogel hists
auto h_aero_should = new TH1D ("P.aero.npeSum","SHMS Carbon 60uA Efficiency - Run 16745", 300, 0.1, 50);
...
// calorimeter hists
auto h_cal_should = new TH1D ("P.cal.etottracknorm", "SHMS Carbon 60uA Efficiency - Run 16745", 300, 0.1, 2);
...
{ 
    if(line.find("run 16745 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16745 - hgcer: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
    if(line.find("run 16745 - ngcer:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16745 - aero: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
}  
...
{
    eff_file_out << "run 16745 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16745 - hgcer: "<< fixed << setprecision(5) << effhgcer << " +/- "  << setprecision(5)<< dEffhgcer << endl;
    eff_file_out << "run 16745 - ngcer: "<< fixed << setprecision(5) << effngcer << " +/- "  << setprecision(5)<< dEffngcer << endl;
    eff_file_out << "run 16745 - aero: "<< fixed << setprecision(5) << effaero << " +/- "  << setprecision(5)<< dEffaero << endl << endl;
    foundhgcer = true;
    foundngcer = true;
    foundcal = true;
    foundaero = true;
...
calLegend->AddEntry(h_cal_should, "Integral = 514846.00 +/- 717.53 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 497127.00 +/- 705.079", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9656 +/- 0.0027 ", "");
...
hgcerLegend->AddEntry(h_hgcer_should, "Integral = 468761.00 +/- 684.66","l"); // hgcer should

hgcerLegend->AddEntry(h_hgcer_did,"Did","l");
hgcerLegend->AddEntry(h_hgcer_did,"Integral = 456021.00 +/- 675.29", "l"); // hgcer did

hgcerLegend->AddEntry( h_hgcer_did,"Efficiency = 0.9728 +/- 0.0029", "");
...
ngcerLegend->AddEntry(h_ngcer_should, "Integral = 490098.00 +/- 700.07","l"); // ngcer should

ngcerLegend->AddEntry(h_ngcer_did,"Did","l");
ngcerLegend->AddEntry(h_ngcer_did,"Integral = 464394.00 +/- 681.46", "l"); // ngcer did

ngcerLegend->AddEntry( h_ngcer_did,"Efficiency = 0.9476 +/- 0.0027", "");
...
aeroLegend->AddEntry(h_aero_should, "Integral = 468761.00 +/- 684.66","l"); // aero should

aeroLegend->AddEntry(h_aero_did,"Did","l");
aeroLegend->AddEntry(h_aero_did,"Integral = 456021.00 +/- 675.29", "l"); // aero did

aeroLegend->AddEntry( h_aero_did,"Efficiency = 0.9728 +/- 0.0029 ", "");
...
c1->Print("16745_hgcer_Eff.pdf","pdf");
c2->Print("16745_cal_Eff.pdf","pdf");
c3->Print("16745_ngcer_Eff.pdf","pdf");
c4->Print("16745_aero_Eff.pdf", "pdf");
}