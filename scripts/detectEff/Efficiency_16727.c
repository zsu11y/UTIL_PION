...
void Efficiency_16727()
{

// Run 16727 - HMS only, Carbon, 80 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16727_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS Carbon 80uA Efficiency - Run 16727", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS Carbon 80uA Efficiency - Run 16727", 300, 0.1, 2);
...
{ 
    if(line.find("run 16727 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16727 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16727 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16727 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
calLegend->AddEntry(h_cal_should, "Integral = 367659.00 +/- 606.35","l"); //cal should

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 366731.00 +/- 605.58", "l"); // cal did

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9975 +/- 0.0033", "");
...
cerLegend->AddEntry(h_cer_should, "Integral = 367274.00 +/- 606.03","l"); //cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 366762.00 +/- 605.61", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9986 +/- 0.0033", "");
...
c1->Print("16727_cer_Eff.pdf","pdf");
c2->Print("16727_cal_Eff.pdf","pdf");
