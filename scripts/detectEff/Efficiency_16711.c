...
void Efficiency_16711()
{

// Run 16711 - HMS only, LH2, 2.5 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16711_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS LH2 2.5uA Efficiency - Run 16711", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS LH2 2.5uA Efficiency - Run 16711", 300, 0.1, 2);
...
{ 
    if(line.find("run 16711 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16711 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16711 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16711 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
c1->Print("16711_cer_Eff.pdf","pdf");
c2->Print("16711_cal_Eff.pdf","pdf");
}