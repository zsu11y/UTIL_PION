...
void Efficiency_16731()
{

// Run 16731 - HMS only, Carbon, 25 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16731_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS Carbon 25uA Efficiency - Run 16731", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS Carbon 25uA Efficiency - Run 16731", 300, 0.1, 2);
...
{ 
    if(line.find("run 16731 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16731 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16731 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16731 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
calLegend->AddEntry(h_cal_should, "Integral = 843612.00 +/- 918.48 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 841483.00 +/- 917.32 ", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9975 +/- 0.0022", "");
...
cerLegend->AddEntry(h_cer_should, "Integral = 842382.00 +/- 917.81","l"); // cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 841378.00 +/- 917.27", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9988 +/- 0.0022", "");
...
c1->Print("16731_cer_Eff.pdf","pdf");
c2->Print("16731_cal_Eff.pdf","pdf");
}