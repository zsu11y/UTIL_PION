...
void Efficiency_16709()
{

// Run 16709 - HMS only, LH2, 15 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16709_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS LH2 15uA Efficiency - Run 16709", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS LH2 15uA Efficiency - Run 16709", 300, 0.1, 2);
...
{ 
    if(line.find("run 16709 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16709 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16709 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16709 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
calLegend->AddEntry(h_cal_should, "Integral = 563547.00 +/- 750.70 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 562052.00 +/- 749.70 ", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9973 +/- 0.0027 ", "");
...
cerLegend->AddEntry(h_cer_should, "Integral = 562880.00 +/- 750.25","l"); // cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 562118.00 +/- 749.75", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9986 +/- 0.0027", "");
...
c1->Print("16709_cer_Eff.pdf","pdf");
c2->Print("16709_cal_Eff.pdf","pdf");
}