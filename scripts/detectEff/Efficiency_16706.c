...
void Efficiency_16706()
{

// Run 16706 - HMS only, LH2, 40 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16706_-1.root"); //open a new file for each beam current 
...
//cerenkov hists
auto h_cer_should = new TH1D ("H.cer.npeSum", "HMS LH2 40uA Efficiency - Run 16706", 300 , 0.1, 40);
...
// calorimeter hists
auto h_cal_should = new TH1D ("H.cal.etottracknorm", "HMS LH2 40uA Efficiency - Run 16706", 300, 0.1, 2);
...
{ 
    if(line.find("run 16706 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16706 - cer: ")!= std::string::npos)
    {
        foundcer = true;
    }
}  
...
{
    eff_file_out << "run 16706 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16706 - cer: "<< fixed << setprecision(5) << effcer << " +/- "  << setprecision(5)<< dEffcer << endl << endl;
...
calLegend->AddEntry(h_cal_should, "Integral = 534373.00 +/- 731.01 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 532802.00 +/- 729.93 ", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9971 +/- 0.0027 ", "");
...
cerLegend->AddEntry(h_cer_should, "Integral =  534089.00 +/- 730.81","l"); // cer should

cerLegend->AddEntry(h_cer_did,"Did","l");
cerLegend->AddEntry(h_cer_did,"Integral = 533031.00 +/- 730.09", "l"); // cer did

cerLegend->AddEntry( h_cer_did,"Efficiency = 0.9980 +/- 0.0027 ", "");
...
c1->Print("16706_cer_Eff.pdf","pdf");
c2->Print("16706_cal_Eff.pdf","pdf");
}