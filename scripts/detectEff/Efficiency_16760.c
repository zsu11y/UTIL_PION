...
void Efficiency_16760()
{

// Run 16760 - SHMS only, LH2, 40 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16760_-1.root"); //open a new file for each beam current 
...
// hg cerenkov hists
auto h_hgcer_should = new TH1D ("P.hgcer.npeSum", "SHMS LH2 40uA Efficiency - Run 16760", 300 , 0.1, 40);
...
// ng cerenkov hists
auto h_ngcer_should = new TH1D ("P.ngcer.npeSum", "SHMS LH2 40uA Efficiency - Run 16760", 300 , 0.1, 40);
...
// aerogel hists
auto h_aero_should = new TH1D ("P.aero.npeSum","SHMS LH2 40uA Efficiency - Run 16760", 300, 0.1, 50);
...
// calorimeter hists
auto h_cal_should = new TH1D ("P.cal.etottracknorm", "SHMS LH2 40uA Efficiency - Run 16760", 300, 0.1, 2);
...
{ 
    if(line.find("run 16760 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16760 - hgcer: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
    if(line.find("run 16760 - ngcer:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16760 - aero: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
}  
...
{
    eff_file_out << "run 16760 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16760 - hgcer: "<< fixed << setprecision(5) << effhgcer << " +/- "  << setprecision(5)<< dEffhgcer << endl;
    eff_file_out << "run 16760 - ngcer: "<< fixed << setprecision(5) << effngcer << " +/- "  << setprecision(5)<< dEffngcer << endl;
    eff_file_out << "run 16760 - aero: "<< fixed << setprecision(5) << effaero << " +/- "  << setprecision(5)<< dEffaero << endl << endl;
    foundhgcer = true;
    foundngcer = true;
    foundcal = true;
    foundaero = true;
...
calLegend->AddEntry(h_cal_should, "Integral = 509568.00 +/- 713.84 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 489344.00 +/- 699.53", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9603 +/- 0.0027 ", "");
...
hgcerLegend->AddEntry(h_hgcer_should, "Integral = 459492.00 +/- 677.86","l"); // hgcer should

hgcerLegend->AddEntry(h_hgcer_did,"Did","l");
hgcerLegend->AddEntry(h_hgcer_did,"Integral = 447690.00 +/- 669.10", "l"); // hgcer did

hgcerLegend->AddEntry( h_hgcer_did,"Efficiency = 0.9743 +/- 0.0029", "");
...
ngcerLegend->AddEntry(h_ngcer_should, "Integral = 480681.00 +/- 693.31","l"); // ngcer should

ngcerLegend->AddEntry(h_ngcer_did,"Did","l");
ngcerLegend->AddEntry(h_ngcer_did,"Integral = 457334.00 +/- 676.26", "l"); // ngcer did

ngcerLegend->AddEntry( h_ngcer_did,"Efficiency = 0.9514 +/- 0.0028", "");
...
aeroLegend->AddEntry(h_aero_should, "Integral = 459492.00 +/- 677.86","l"); // aero should

aeroLegend->AddEntry(h_aero_did,"Did","l");
aeroLegend->AddEntry(h_aero_did,"Integral = 447690.00 +/- 669.10", "l"); // aero did

aeroLegend->AddEntry( h_aero_did,"Efficiency = 0.9743 +/- 0.0029 ", "");
...
c1->Print("16760_hgcer_Eff.pdf","pdf");
c2->Print("16760_cal_Eff.pdf","pdf");
c3->Print("16760_ngcer_Eff.pdf","pdf");
c4->Print("16760_aero_Eff.pdf", "pdf");
}