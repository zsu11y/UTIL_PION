...
void Efficiency_16739()
{

// Run 16739 - SHMS only, Carbon, 2.5 uA

TFile *f1 = new TFile("/u/group/c-pionlt/USERS/zsully/hallc_replay_lt/UTIL_PION/ROOTfiles/Analysis/Lumi/PionLT_replay_luminosity_16739_-1.root"); //open a new file for each beam current 
...
// hg cerenkov hists
auto h_hgcer_should = new TH1D ("P.hgcer.npeSum", "SHMS Carbon 2.5uA Efficiency - Run 16739", 300 , 0.1, 40);
...
// ng cerenkov hists
auto h_ngcer_should = new TH1D ("P.ngcer.npeSum", "SHMS Carbon 2.5uA Efficiency - Run 16739", 300 , 0.1, 40);
...
// aerogel hists
auto h_aero_should = new TH1D ("P.aero.npeSum","SHMS Carbon 2.5uA Efficiency - Run 16739", 300, 0.1, 50);
...
// calorimeter hists
auto h_cal_should = new TH1D ("P.cal.etottracknorm", "SHMS Carbon 2.5uA Efficiency - Run 16739", 300, 0.1, 2);
...
auto bcmAvgCut1 = 2.5; // run replay lumi to figure out cut values for this run
auto bcmAvgCut2 = 1;
...
{ 
    if(line.find("run 16739 - cal:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16739 - hgcer: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
    if(line.find("run 16739 - ngcer:")!= std::string::npos)
    {
        foundcal = true;
    }
    if(line.find ("run 16739 - aero: ")!= std::string::npos)
    {
        foundhgcer = true;
    }
}  
...
{
    eff_file_out << "run 16739 - cal: "<< fixed << setprecision(5) << effcal << " +/- "  << setprecision(5)<< dEffcal << endl;
    eff_file_out << "run 16739 - hgcer: "<< fixed << setprecision(5) << effhgcer << " +/- "  << setprecision(5)<< dEffhgcer << endl;
    eff_file_out << "run 16739 - ngcer: "<< fixed << setprecision(5) << effngcer << " +/- "  << setprecision(5)<< dEffngcer << endl;
    eff_file_out << "run 16739 - aero: "<< fixed << setprecision(5) << effaero << " +/- "  << setprecision(5)<< dEffaero << endl << endl;
    foundhgcer = true;
    foundngcer = true;
    foundcal = true;
    foundaero = true;
...
calLegend->AddEntry(h_cal_should, "Integral = 335506.00 +/- 579.23 ","l"); // cal should 

calLegend->AddEntry(h_cal_did,"Did","l");
calLegend->AddEntry(h_cal_did,"Integral = 324212.00 +/- 569.40", "l"); // cal did 

calLegend->AddEntry( h_cal_did,"Efficiency = 0.9663 +/- 0.0034", "");
...
hgcerLegend->AddEntry(h_hgcer_should, "Integral = 306496.00 +/- 553.62","l"); // hgcer should

hgcerLegend->AddEntry(h_hgcer_did,"Did","l");
hgcerLegend->AddEntry(h_hgcer_did,"Integral = 298050.00 +/- 545.94", "l"); // hgcer did

hgcerLegend->AddEntry( h_hgcer_did,"Efficiency = 0.9724 +/- 0.0035", "");
...
ngcerLegend->AddEntry(h_ngcer_should, "Integral = 319895.00 +/- 565.59","l"); // ngcer should

ngcerLegend->AddEntry(h_ngcer_did,"Did","l");
ngcerLegend->AddEntry(h_ngcer_did,"Integral = 302879.00 +/- 550.34", "l"); // ngcer did

ngcerLegend->AddEntry( h_ngcer_did,"Efficiency = 0.9468 +/- 0.0034", "");
...
aeroLegend->AddEntry(h_aero_should, "Integral = 306496.00 +/- 553.62","l"); // aero should

aeroLegend->AddEntry(h_aero_did,"Did","l");
aeroLegend->AddEntry(h_aero_did,"Integral = 298050.00 +/- 545.94", "l"); // aero did

aeroLegend->AddEntry( h_aero_did,"Efficiency = 0.9724 +/- 0.0035", "");
...
c1->Print("16739_hgcer_Eff.pdf","pdf");
c2->Print("16739_cal_Eff.pdf","pdf");
c3->Print("16739_ngcer_Eff.pdf","pdf");
c4->Print("16739_aero_Eff.pdf", "pdf");
}